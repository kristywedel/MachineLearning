...
#remove hilghly correlated fields
...
The random forest model ranks the importance of fields in the model. It can be used to identify some of the more useful predictors.
...
Create the random forest model using the best number of trees 

```{r}
fit<-randomForest(classe~., data=pml.training, ntree=1500, importance = TRUE)
print(fit)
pred_val_test<-predict(fit, pml.validation)
confusionMatrix(pred_val_test, pml.validation$classe)$overall[1]
```
